> D'après nos informations, un criminel que nous cherchons depuis plusieurs mois se rendrait au Festival de Cannes afin d'assister à la projection de films… et de vendre des informations sensibles. Nous avons réussi à intercepter deux communications qui semblent intéressantes, un appel et un message.
Vous devez identifier le pays dans lequel il se cache actuellement, localiser où la vente va avoir lieu, et ce qui va être échangé.
Format du flag : SHLK{Pays_Lieu_Objet} Exemple : SHLK{Etats-Unis_Café-La-Perle_Code}
> 
> Avec un audio et un .txt.

Audio avec BirdNET : Langrayen brun ? ou similaire

Texte contenant : 
“Le dernier restaurant était très agréable, l'ambiance détendue. On devrait refaire ça, c'est toujours plaisant.
J'ai beaucoup ri en parlant de ce fameux chien. Les projets ne manquent pas, c'est certain.
Peut-être qu'en 2026 on se retrouvera à nouveau ; code pour la date : T30-xkc-8M.“
+ des lignes (stéganographie par espaces et tabulations)

-> https://www.dcode.fr/langage-whitespace (convertir en .ws d'abord) donne : 
“Prochain rendez-vous au Lounge Bar pour l'échange des clefs”

Donc format SHLK{pays_Lounge-Bar_Clefs}...

En réalité, avec un meilleur micro, l'application aurait dû donner Echo Parakeet, aias Perruche de Maurice.
